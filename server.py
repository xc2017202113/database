...
def hello_world ():
    ...
    if password != password_real:
        ...
        response.set_cookie ('user', username, max_age=2 * 3600)
        return response
...
def analyse ():
    ...
    charts_paras = {"years_money_accumulate": years_money_accumulate,
                    "company_money_accumulate": company_money_accumulate,
                    "trade_type_accmuldate": trade_type_accmuldate, "relation_accumulate": relation_accumulate}

    return render_template ("analyse.html", socket_code=socket_code, start_years=start_year, end_years=end_year,
                            accumulate_company=count_accumulate, accumulate_trade_money=count_money, \
...
def analyse_index ():
    ...
    return render_template ("analyse_index_charts.html", socket_code=socket_code, start_years=start_year,
                            end_years=end_year, \
                            accumulate_CR_5_index=accumulate_CR_5_index, accumulate_Z_index=accumulate_Z_index, \
                            accumulate_Herfindahl_5_index=accumulate_Herfindahl_5_index,
...
@app.route ('/analyse_meeting', methods=["GET", "POST"])
def analyse_meeting ():
    socket_code = int (request.args.get ("socket_code"))

    start_year = request.args.get ("start_years")

    sql_operation = 'select 第一大股东持股比例,第一大股东持股数量,第二大股东持股比例,第二大股东持股数量,第三大股东持股比例,第三大股东持股数量,' \
                    + '第一大流通股东持股比例,第一大流通股东持股数量,第二大流通股东持股比例,第二大流通股东持股数量,第三大流通股东持股比例,第三大流通股东持股量 from 上市公司治理结构_股东持股信息 ' \
                    + 'where 股票代码={} and 年度 = {} ;'.format (socket_code, start_year)
    ...
    result = connection.execute (sql_operation)
    for row in result:
        noflow_total = float (row["第一大股东持股数量"]) + float (row["第二大股东持股数量"]) + float (row["第三大股东持股数量"])
        flow_total = float (row["第一大流通股东持股数量"]) + float (row["第二大流通股东持股数量"]) + float (row["第三大流通股东持股量"])
        total = noflow_total + flow_total

        noflow_change["第一股东"] = float (row["第一大股东持股数量"])
        noflow_change["第二股东"] = float (row["第二大股东持股数量"])
        noflow_change["第三股东"] = float (row["第三大股东持股数量"])

        flow_change["第一股东"] = float (row["第一大流通股东持股数量"])
        flow_change["第二股东"] = float (row["第二大流通股东持股数量"])
        flow_change["第三股东"] = float (row["第三大流通股东持股量"])

        noflow_scale_change["第一股东"] = float (row["第一大股东持股比例"])
        noflow_scale_change["第二股东"] = float (row["第二大股东持股比例"])
        noflow_scale_change["第三股东"] = float (row["第三大股东持股比例"])

        flow_scale_change["第一股东"] = float (row["第一大流通股东持股比例"])
        flow_scale_change["第二股东"] = float (row["第二大流通股东持股比例"])
        flow_scale_change["第三股东"] = float (row["第三大流通股东持股比例"])

    charts_paras = {"noflow_scale_change": noflow_scale_change, \
                    "flow_change": flow_change, \
                    "noflow_change": noflow_change, \
                    "flow_scale_change": flow_scale_change}
    return render_template ("analyse_meeting_charts.html", socket_code=socket_code, start_years=start_year,
                            noflow_total=int (noflow_total), \
                            flow_total=int (flow_total), total=int (total), \
 \
                            charts_paras=json.dumps (charts_paras))
...
def modify_frame ():
    ...
    sql = "select authortity from user where userName=\"{}\"".format (name)
    result = connection.execute (sql)

    authortity = 0
    for row in result:
        authortity = int (row["authortity"])
...
def find_balanced_sheet_data ():
    ...
    for row in result:
        ...
        for index, i in enumerate (notflow_assert_varibles):
            now_years_notflow_assert_varibles_dict["now_years_notflow_assert_varibles_dict_{}".format (index + 1)] = \
            row[i]
    ...
    for row in result:
        ...
        for index, i in enumerate (notflow_assert_varibles):
            last_years_notflow_assert_varibles_dict["last_years_notflow_assert_varibles_dict_{}".format (index + 1)] = \
            row[i]
        ...
        for index, i in enumerate (owner_equity_varibles):
            last_years_owner_equity_varibles_dict["last_years_owner_equity_varibles_dict_{}".format (index + 1)] = row[
                i]
